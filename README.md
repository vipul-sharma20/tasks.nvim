...
Tasks belong in the markdown files where the work is documented like diary
entries, project notes, meeting logs and not in a separate database.

tasks.nvim treats the markdown as the source of truth and builds a queryable
dashboard on top of it with ripgrep. A task is just a checkbox with #task on
it. Other metadata like due dates, priorities, labels, linked notes is optional
metadata on the same line. The plugin is a view layer, not a data layer.
